...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Add this check to determine if it's the user's own profile
        const currentUserId = localStorage.getItem('userId');
        setIsOwnProfile(currentUserId === sellerId);
      } catch (error) {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    const feedbackResponse = await fetch(
      `http://localhost:5000/api/feedback/user/${sellerId}`,
      {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {!isOwnProfile && (
              <button
                ...
                className="bg-[#438951] text-white px-4 py-2 rounded-lg hover:bg-[#357641]"
              >
                Leave Feedback
              </button>
            )}
